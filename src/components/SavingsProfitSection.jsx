    ...
    <div className="rounded-2xl overflow-hidden flex flex-col" style={{ background: "#F5F5F0" }}>
      <div style={{ padding: "20px 20px 0 20px", flex: 1 }}>
        ...
        <div style={{ display: "flex", paddingTop: "83px" }}>
          <div style={{
            width: Y_AXIS_W, height: BAR_ZONE, marginBottom: LABEL_ZONE,
            flexShrink: 0, display: "flex", flexDirection: "column",
            justifyContent: "space-between", alignItems: "flex-end",
            paddingRight: 6, fontSize: 11, color: "#9ca3af",
          }}>
            {Y_LABELS.map((v) => (
          ...
          <div style={{ flex: 1, position: "relative", height: BAR_ZONE + LABEL_ZONE }}>
            {Y_LABELS.map((v, i) => (
              <div key={`grid-${v}`} style={{
                position: "absolute", top: (i / (TICKS - 1)) * BAR_ZONE,
                left: 0, right: 0, borderTop: "1.5px dashed #d1d5db", zIndex: 0,
              }} />
            ))}
            ...
            {barDefs.map(({ costVal, revVal, label }, idx) => {
              ...
              return (
                <div key={label} style={{
                  ...
                  width: BAR_W, height: BAR_ZONE,
                  display: "flex", flexDirection: "column",
                  justifyContent: "flex-end", zIndex: 1,
                }}>
                  {rH > 0 && (
                    <div style={{ width: BAR_W, height: rH, flexShrink: 0, background: "#1A9E75", borderRadius: "6px 6px 0 0" }} />
                  )}
                  <div style={{ width: BAR_W, height: cH, flexShrink: 0, background: "#E8A838", borderRadius: rH > 0 ? "0 0 6px 6px" : "6px" }} />
                </div>
              );
            })}
            ...
            {barDefs.map(({ label }, idx) => (
              <p key={`lbl-${label}`} style={{
                position: "absolute", top: BAR_ZONE,
                left: idx * (BAR_W + BAR_GAP), width: BAR_W, height: LABEL_ZONE,
            ...
            <div style={{
              ...
              height: BAR_ZONE, width: "auto", zIndex: 1, pointerEvents: "none",
            }}>
              <img
                src={farmerImg} alt="Farmer"
                style={{ height: "105%", width: "auto", objectFit: "contain", objectPosition: "bottom", display: "block" }}
              />
            </div>
          </div>
        </div>
...
const SavingsPage = ({ hideHeading = false }) => {
  ...
  return (
    <div
      ...
      style={{ paddingTop: hideHeading ? 0 : "2rem", paddingBottom: "4rem" }}
    >
      {!hideHeading && (
